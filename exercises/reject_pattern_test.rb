...
class RejectPatternTest < Minitest::Test
  ...
  def test_remove_vowels
    ...
    letters.each do |letter|
      remaining << letter unless letter.include?('aeiou')
    end
  ...
  def test_remove_words_with_more_than_three_letters
    ...
    selected = []
    words.each do |word|
      selected << word unless word.length >= 4
    end
  ...
  def test_remove_animals_starting_with_vowels
    ...
    remaining = []
    animals.each do |animal|
      remaining << animal unless animal.start_with?("aeiou") == false
    end
  ...
  def test_remove_capitalized_words
    ...
    remaining = []
    words.each do |word|
      remaining << word unless word.upcase!
    end
    assert_equal ["dog", "Trev  or", "butter"], remaining
  end
